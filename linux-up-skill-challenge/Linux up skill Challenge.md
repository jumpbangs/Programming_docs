...
- `cat`to dump the contents of a file.

- `less`to navigate the file, up and down with arrow files, you can also navigate using `gg`and `G` to hop to the top and bottom of file.  

- `tail`to print out the end of file or `tail -f` to print and log the file in real-time if you're logging the file.
- To dump and search out a search item using `cat` and `grep` with the following: `cat /var/log/auth.log | grep "<term_that_your_searching>"`
- `cut` to select out portions each line by specifying `-d`(delimiter) and `-f`(field) example: `gerp "<search_term>" "file"| grep "root" | cut-f 10 -d`.
- `wc`to count line and letter of files.

###### Look into `awk` and `sed` 



## Day 9 

- `ss` - this, `socket status` is a standard utility  replacing `netstat`.
- `namp` - this `port scaner` won't default.
- `ss -tlp` to show the output of 
